    ...
    while (Date.now() - startTime < timeout) {
        try {
            ...
            if (imageResult && imageResult.x !== 0 && imageResult.y !== 0 && imageResult.width !== 0 && imageResult.height !== 0) {
                ...
                log.info(`成功识别图像，坐标:text=${imageResult.text}, x=${imageResult.x}, y=${imageResult.y}, width=${imageResult.width}, height=${imageResult.height}`);
                return {
                    success: true,
                    x: imageResult.x,
                    y: imageResult.y,
...
(async function () {
        ...
        for (const imagePath of imagePaths) {
            ...
            if (result.success) {
                // 输出坐标和识图范围推荐
                ImageTextUtils.info(`图标 ${fileName} 的坐标: x=${result.x}, y=${result.y}, width=${result.width}, height=${result.height}，识图范围推荐: ${result.x - 1}, ${result.y - 1}, ${result.width + 2}, ${result.height + 2}`,true);
                let ObjectOcr = ImageTextUtils.recognitionObjectOcr(result.x, result.y, result.width, result.height)
                let GameRegion=ImageTextUtils.openCaptureGameRegion()
                let list=ImageTextUtils.findMultiByCaptureGameRegion(GameRegion,ObjectOcr)
                for (let res of list) {
                    ImageTextUtils.info(`识别到文字: ${res.text}, x=${res.x}, y=${res.y}, width=${res.width}, height=${res.height}`,true);
                }
            } else {
                // log.warn(`未能识别到图标 ${fileName}`);
            }

            await ImageTextUtils.wait(50); // 确保识别之间有足够的时间间隔
        }
})();
